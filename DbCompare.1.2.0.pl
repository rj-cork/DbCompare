...
my %CHILDREN;
my $SKIP_PARTS_ROW_LIMIT = 50;# Lib::Database::RESULT_BATCH_SIZE*1000;
...
#???		#select count(*) from DATA_OWNER.TAB partition (SYS_P29878);
	...
	print STDERR Dumper($new_vparts);
				for (my $vp = 0; $vp < scalar(@{$new_vparts}); $vp++ ) {
					...
					$h{WHERE} = $new_vparts->[$vp];
					$list{$row->{OWNER}}->{$row->{TABLE_NAME}}->{'vp'.$vp} = \%h;
				}

			} else {
				$list{$row->{OWNER}}->{$row->{TABLE_NAME}} = '';
			}

		} else { #partitions are here
			#always use name - its only for basedb
	...
	print STDERR ">$dbname>",$row->{P_NUM_ROWS}," -- $p\n";
			...
			#partition too big, need to split it
				...
				next if ($dbname ne $CONFIG{'base_db'}); #we are checkinging boundaries for "virtual partitions" only in "primary" database
				...
				for (my $vp = 0; $vp < scalar(@{$new_vparts}); $vp++ ) {
					...
					$h{PNAME} = $row->{PARTITION_NAME};
					$h{WHERE} = $new_vparts->[$vp];
					$h{PARTITION_POSITION} = $row->{PARTITION_POSITION};
					$h{HIGH_VALUE} = $row->{HIGH_VALUE} if (defined($row->{HIGH_VALUE}));
					$list{$row->{OWNER}}->{$row->{TABLE_NAME}}->{$row->{PARTITION_NAME}}->{'vsp'.$vp} = \%h;
				}
				...
				$list{$row->{OWNER}}->{$row->{TABLE_NAME}}->{$row->{PARTITION_NAME}} = \%h;
			}
		}
...
sub GetObjects {
	...
	#put base_db in front of the list
	my @dbnames = keys %{$dbs};
	for (my $i=0; $i<scalar(@dbnames); $i++) {
		if ($dbnames[$i] eq $CONFIG{'base_db'}) {
			$dbnames[$i] = $dbnames[0];
			$dbnames[0] = $CONFIG{'base_db'};
			last;
		}
	}

	#instead reading objects from all databases, we will create list only using PRIMARY (in case mapping it would be hard to determine schemas)
	#foreach my $db (@dbnames) {
	my $db = $CONFIG{'base_db'};
	my $dbh = Lib::Database::Connect($dbs->{$db}, 'GetObjects');

	if (not defined($dbh)) {
		PrintMsg(ERROR, "Connection failed to $db\n");
		exit 1;
	}

	foreach my $o (@{$objects_in}) { #schema[.table[.partition]]
		my $h_ref;

			# $1 (obligatory) - schema, $2 (optional) - table, TODO: $3 (optional) - partition
		if ($o =~ /^([\w\d\?\*%]+)(?:\.([\w\d\$\?\*%]+))?$/) { 
			$h_ref = GetObjectList($dbh, $db, \%objects_all, $1, $2);
		} else {
...
sub main {
	...
	($databases, $inputlist) = GetParams();

	SetProcessName(@ARGV);

	if ($CONFIG{'state_file'}) {  #for now we are reprocessing old list
		($objectlist, $report) = LoadStateFile($CONFIG{'state_file'}, $databases);
	}


	CreatePidfile($CONFIG{'pid_file'});

	if (not defined($objectlist)) { #this is fresh start, no tables in statefile or no statefile
