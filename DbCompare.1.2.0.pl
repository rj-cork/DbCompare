...
#???		#select count(*) from DATA_OWNER.TAB partition (SYS_P29878);
	...
	foreach my $row (@{$r}) {
		...
		if (not defined ($row->{PARTITION_NAME})) {
			#check if the table was split into virtual partitions for base_db
			if (defined $all_objects->{$CONFIG{'base_db'}} and defined $all_objects->{$CONFIG{'base_db'}}->{"$p.vp0"}) {

				foreach my $v (keys %{$all_objects->{$CONFIG{'base_db'}}}) {
					$list{$v} = $all_objects->{$CONFIG{'base_db'}}->{$v};
				}
				
				next; #go to next table
			} 

			if (defined($row->{T_NUM_ROWS}) && $row->{T_NUM_ROWS} > $SKIP_PARTS_ROW_LIMIT) {
			#table too big, need to split it

			#TODO: ?? generated only for DB0 / basedb, rest has copy of predicates from db0. What about ie hash partitions with system generated names?
	print STDERR ">$dbname>",$row->{T_NUM_ROWS},"\n";
				next if ($dbname ne $CONFIG{'base_db'});
	...
	print STDERR Dumper($new_vparts);
			...
			#check if the table was split into virtual partitions for base_db
#TODO: nie mozna przepisywac, lookup partycji powinien byc po nazwach a jak sa autogenerated to powinien byc po highvalue
	#jak po numerkach to moze sie ponadpisywac niewlasciwie jesli gdzies z przodu zostala np pusta partycja
			if (defined $all_objects->{$CONFIG{'base_db'}} and defined $all_objects->{$CONFIG{'base_db'}}->{"$p.vp0"}) {
